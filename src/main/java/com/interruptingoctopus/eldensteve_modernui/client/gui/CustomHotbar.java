...
public class CustomHotbar implements IRenderable {
    ...
    public void render(GuiGraphics guiGraphics, @Nullable Player player, int x, int y, int selectedIndex) {
        ...
        for (int i = 0; i < 5; i++) {
            int slotX = x + 1 + (i * 20);
            renderSlot(guiGraphics, (LocalPlayer) player, slotX, y, i, (i == selectedIndex));
        }
    }

    private void renderSlot(GuiGraphics guiGraphics, LocalPlayer player, int x, int y, int visualIndex, boolean isSelected) {
        ItemStack itemStack;
        ...
        if (this.alignment == MeterBar.Alignment.LEFT) {
            ...
            if (visualIndex == 0) {
                ...
                itemStack = player.getInventory().getItem(visualIndex - 1); // Slots 0-3
            }
        } else {
            // Right Bar (Main-hand)
        ...
        if (isSelected) {
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, HOTBAR_SELECTION_SPRITE, x - 2, y - 1, 24, 24);
        }
    }
